...
`CONFIG_DIR_NAME` is `.omp` (`packages/utils/src/dirs.ts`). `PI_CONFIG_DIR` changes the OMP user root used by the generic helpers. `PI_CONFIG_ROOT` replaces the home-joined root outright with a full path that must be absolute (or `~`-relative): it applies on every platform, named profiles resolve under `<PI_CONFIG_ROOT>/profiles/<name>`, and it wins over the XDG category redirects so one pinned path keeps the whole tree — data, state, and cache — in one directory. `PI_CODING_AGENT_DIR` is different: for the default profile it changes `getAgentDir()` consumers such as native discovery, settings, and runtime state, but it does **not** change the generic `getConfigDirs()` / `findConfigFile()` OMP base. Named profiles ignore `PI_CODING_AGENT_DIR`.
...
On macOS and Linux, an existing `$XDG_DATA_HOME/omp`, `$XDG_STATE_HOME/omp`, or `$XDG_CACHE_HOME/omp` can relocate the corresponding data, state, or cache paths. For a named profile, OMP uses an XDG category only when that category already contains `omp/profiles/<name>`; otherwise that category remains under `~/.omp/profiles/<name>`. Run `omp config init-xdg` before relying on XDG paths. A `PI_CONFIG_ROOT` pin outranks these redirects: when it is set, every category stays under the pinned root.
